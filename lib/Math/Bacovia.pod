...
In combination with the B<full> option (set to a true value), the returned list may contain thousands or even millions of alternative expressions.
...
It takes the same options as the C<alternatives()> method.

In the current implementation, the simplification of expressions is done by generating all the alternative representations and selecting the shortest expression from this list.

The simplified expression may not always be optimal. In some cases, calling the C<simple()> method on an already simplified expression, the expression may get simplified even further:

    my $expr = $x->simple(full => 1)->simple(full => 1);
...
In the current implementation, the expansion of expressions is done by generating all the alternative representations and selecting the longest expression from this list.
